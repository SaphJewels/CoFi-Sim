...
def drawBackground1():
    ...
    #paying bills
    if(payBills and not (tuition and healthCare and auto)):
            write(100, 100, "Pay your bills", "black")
...
def drawBackground2():
    ...
    if (tuition):
        ...
        write(100, 100, "Tuition - $500", "black")
    if (healthCare):
...
girl = pygame.image.load('assets/girl_front.png')
while True: # main game loop
    if(window == 1):
        drawBackground1()
        DISPLAY.blit(girl, (x, y))
        drawBlanket()
    elif(window == 2):
        drawBackground2()
    elif (window == 3):
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE and window == 2:
                    ...
                    window = 2
            if event.key == K_LEFT:
                girl = pygame.image.load('assets/girl_side.png')
            ...
            #sleep
            if(x == 300):
                if(tuition and healthCare and auto):
                    sleep()        
                    x = 550
                    y = 50
                    weekCount += 1
                    ...
                    if(grades > 1):
                        grades -= 1
                    energy = 10
                    addedHealth = random.randint(3, 5)
                    if(health + addedHealth <= 10):
                        health += addedHealth 
                    subtractedHunger = random.randint(1, 3)
                    if(hunger - subtractedHunger > 0):
                        hunger -= subtractedHunger
